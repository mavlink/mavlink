...
#define MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_MIN_LEN 255
                               ...
                               uint16_t watchdog_id, uint16_t process_id, const char *name, const char *arguments, int32_t timeout)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_MIN_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_CRC);
}
                                   ...
                                   uint16_t watchdog_id,uint16_t process_id,const char *name,const char *arguments,int32_t timeout)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_MIN_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_CRC);
}
...
static inline void mavlink_msg_watchdog_process_info_send(mavlink_channel_t chan, uint16_t watchdog_id, uint16_t process_id, const char *name, const char *arguments, int32_t timeout)
{
    ...
    _mav_put_char_array(buf, 8, name, 100);
    _mav_put_char_array(buf, 108, arguments, 147);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO, buf, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_MIN_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_CRC);
#else
    mavlink_watchdog_process_info_t packet;
    packet.timeout = timeout;
    packet.watchdog_id = watchdog_id;
    ...
    mav_array_memcpy(packet.name, name, sizeof(char)*100);
    mav_array_memcpy(packet.arguments, arguments, sizeof(char)*147);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO, (const char *)&packet, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_MIN_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_watchdog_process_info_send_struct(mavlink_channel_t chan, const mavlink_watchdog_process_info_t* watchdog_process_info)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_watchdog_process_info_send(chan, watchdog_process_info->watchdog_id, watchdog_process_info->process_id, watchdog_process_info->name, watchdog_process_info->arguments, watchdog_process_info->timeout);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO, (const char *)watchdog_process_info, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_MIN_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN? msg->len : MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN;
        memset(watchdog_process_info, 0, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN);
    memcpy(watchdog_process_info, _MAV_PAYLOAD(msg), len);
#endif
}
