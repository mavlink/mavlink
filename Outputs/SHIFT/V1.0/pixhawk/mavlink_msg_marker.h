...
#define MAVLINK_MSG_ID_MARKER_MIN_LEN 26
#define MAVLINK_MSG_ID_171_LEN 26
#define MAVLINK_MSG_ID_171_MIN_LEN 26
                               ...
                               uint16_t id, float x, float y, float z, float roll, float pitch, float yaw)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_MARKER_MIN_LEN, MAVLINK_MSG_ID_MARKER_LEN, MAVLINK_MSG_ID_MARKER_CRC);
}
                                   ...
                                   uint16_t id,float x,float y,float z,float roll,float pitch,float yaw)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_MARKER;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_MARKER_MIN_LEN, MAVLINK_MSG_ID_MARKER_LEN, MAVLINK_MSG_ID_MARKER_CRC);
}
...
static inline void mavlink_msg_marker_send(mavlink_channel_t chan, uint16_t id, float x, float y, float z, float roll, float pitch, float yaw)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_MARKER, buf, MAVLINK_MSG_ID_MARKER_MIN_LEN, MAVLINK_MSG_ID_MARKER_LEN, MAVLINK_MSG_ID_MARKER_CRC);
#else
    mavlink_marker_t packet;
    packet.x = x;
    packet.y = y;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_MARKER, (const char *)&packet, MAVLINK_MSG_ID_MARKER_MIN_LEN, MAVLINK_MSG_ID_MARKER_LEN, MAVLINK_MSG_ID_MARKER_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_marker_send_struct(mavlink_channel_t chan, const mavlink_marker_t* marker)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_marker_send(chan, marker->id, marker->x, marker->y, marker->z, marker->roll, marker->pitch, marker->yaw);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_MARKER, (const char *)marker, MAVLINK_MSG_ID_MARKER_MIN_LEN, MAVLINK_MSG_ID_MARKER_LEN, MAVLINK_MSG_ID_MARKER_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_MARKER_LEN? msg->len : MAVLINK_MSG_ID_MARKER_LEN;
        memset(marker, 0, MAVLINK_MSG_ID_MARKER_LEN);
    memcpy(marker, _MAV_PAYLOAD(msg), len);
#endif
}
