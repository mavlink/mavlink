...
class MAVLink_radio_message(MAVLink_message):
        ...
        '''
        def __init__(self, rssi, remrssi, rxerrors, serrors, fixed, txbuf):
                MAVLink_message.__init__(self, MAVLINK_MSG_ID_RADIO, 'RADIO')
                self._fieldnames = ['rssi', 'remrssi', 'rxerrors', 'serrors', 'fixed', 'txbuf']
                self.rssi = rssi
                self.remrssi = remrssi
                self.rxerrors = rxerrors
                self.serrors = serrors
                self.fixed = fixed
                self.txbuf = txbuf

        def pack(self, mav):
                return MAVLink_message.pack(self, mav, 128, struct.pack('<HHHHBB', self.rxerrors, self.serrors, self.fixed, self.txbuf, self.rssi, self.remrssi))
...
mavlink_map = {
        ...
        MAVLINK_MSG_ID_RADIO : ( '<HHHHBB', MAVLink_radio_message, [4, 5, 0, 1, 2, 3], 128 ),
...
class MAVLink(object):
        ...
        def radio_encode(self, rssi, remrssi, rxerrors, serrors, fixed, txbuf):
                '''
                Status generated by radio
                ...
                serrors                   : serial errors (uint16_t)
                fixed                     : count of error corrected packets (uint16_t)
                txbuf                     : number of bytes available in transmit buffer (uint16_t)

                '''
                msg = MAVLink_radio_message(rssi, remrssi, rxerrors, serrors, fixed, txbuf)
                msg.pack(self)
                return msg
            
        def radio_send(self, rssi, remrssi, rxerrors, serrors, fixed, txbuf):
                '''
                Status generated by radio
                ...
                '''
                return self.send(self.radio_encode(rssi, remrssi, rxerrors, serrors, fixed, txbuf))
