...
class App(object):
    ...
    def next_message(self):
        ...
        pos = old_div(float(self.mlog.f.tell()), self.filesize)
        self.slider.set(pos)
        ...
        if msg.get_type() == "GPS_RAW_INT":
            self.fdm.set('latitude', old_div(msg.lat,1.0e7), units='degrees')
            self.fdm.set('longitude', old_div(msg.lon,1.0e7), units='degrees')
            if args.gpsalt:
                self.fdm.set('altitude', old_div(msg.alt,1.0e3), units='meters')
