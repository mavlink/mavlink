...
class mavudp_multicast(mavfile):
    '''a UDP multicast mavlink socket'''
    def __init__(self, device, input=True, broadcast=False, source_system=255, use_native=default_native):
        a = device.split(':') # udpmcast:236.10.10.12:12345:eth0
        if len(a) != 3:
            print("UDP ports must be specified as host:port:interface")
            sys.exit(1)
        ...
        multicast_addr = a[0]
        multicast_port = int(a[1])
        interface_name = a[2]


        self.destination_addr = (multicast_addr, multicast_port)

        local_ip = self.get_local_ip_for_interface(interface_name)

        # print("Create socket on %s:%s:%s for my ip: %s" % (interface_name, multicast_addr, multicast_port, local_ip))

        self.port = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

        if input == True:
            # Incomming multicast socket , server
            serveraddress = ('', multicast_port)

            # Set address to be reusable
            self.port.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

            # Bind to the server address
            self.port.bind(serveraddress)

            # Tell the operating system to add the socket to the multicast group
            # on all interfaces.
            # group = socket.inet_aton(multicast_addr)
            # mreq = struct.pack('4sL', group, socket.INADDR_ANY)
            # self.port.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, mreq) # [Errno 19] No such device

            # Construct a membership request...tells router what multicast group we want to subscribe to
            #membership_request = socket.inet_aton(multicast_ip) + struct.pack('=I', socket.INADDR_ANY)
            membership_request = socket.inet_aton(multicast_addr) + socket.inet_aton(local_ip)

            # Send add membership request to socket
            # See http://www.tldp.org/HOWTO/Multicast-HOWTO-6.html for explanation of sockopts
            self.port.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, membership_request)



        else:
            # Outgoing multicast socket , client

            # Set the time-to-live for messages to 1 so they do not go past the
            # local network segment.
            ttl = struct.pack('b', 1)
            self.port.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_TTL, ttl)
    ...
    def get_local_ip_for_interface(self, ifname):
        if ifname not in ni.interfaces():
            print ('Interface %s is not an unknown interface' % ifname)
            print ('Please use an interface from the following list:')
            print (ni.interfaces())
            return '127.0.0.1'

        ifaddresses = ni.ifaddresses(ifname)

        if ni.AF_INET not in ifaddresses:
            print ('Unable to find a IPV4 address for interface %s' % ifname)
            return '127.0.0.1'

        ipadrress = ifaddresses[ni.AF_INET][0]['addr']
        return ipadrress

        # {18: [{'addr': 'a4:5e:60:e6:fd:5d'}],
        #  2: [{'broadcast': '192.168.2.255', 'netmask': '255.255.255.0', 'addr': '192.168.2.80'}],
        #  30: [{'netmask': 'ffff:ffff:ffff:ffff::/64', 'flags': 0L, 'addr': 'fe80::a65e:60ff:fee6:fd5d%en0'},
        #       {'netmask': 'ffff:ffff:ffff:ffff::/64', 'flags': 64L, 'addr': '2a02:a446:5c53:1:a65e:60ff:fee6:fd5d'},
        #       {'netmask': 'ffff:ffff:ffff:ffff::/64', 'flags': 192L, 'addr': '2a02:a446:5c53:1:2585:c0cf:e7ca:1d18'},
        #       {'netmask': 'ffff:ffff:ffff:ffff::/64', 'flags': 72L, 'addr': 'fd81:7662:eaf6:1:a65e:60ff:fee6:fd5d'},
        #       {'netmask': 'ffff:ffff:ffff:ffff::/64', 'flags': 200L, 'addr': 'fd81:7662:eaf6:1:a538:5825:b9aa:84c5'}]}
...
def mavlink_connection(device, baud=115200, source_system=255,
    ...
    if device.startswith('udpmcast:'):
        return mavudp_multicast(device[9:], input=True, source_system=source_system, use_native=use_native, broadcast=True)
