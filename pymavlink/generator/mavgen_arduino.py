...
def generate_MAVLinkArduino_h(directory, xml):
    '''generate MAVLinkArduino.h'''
    f = open(os.path.join(directory, "MAVLinkArduino.h"), mode='w')
    t.write(f,'''
/** @file
 *	@brief MAVLink comm protocol built from ${basename}.xml
 *	@see http://pixhawk.ethz.ch/software/mavlink
...
def generate(basename, xml_list):
    ...
    basename = os.path.join(basename, 'MAVLinkArduino')

    for xml in xml_list:
        generate_one(basename, xml)
    generate_MAVLinkArduino_h(basename, xml)
