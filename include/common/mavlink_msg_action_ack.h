						       ...
						       uint8_t action, uint8_t result)
{
	...
	put_uint8_t_by_index(action, 0,  msg->payload); // The action id
	put_uint8_t_by_index(result, 1,  msg->payload); // 0: Action DENIED, 1: Action executed
...
#else
	memcpy(action_ack, msg->payload, 2);
#endif
}
