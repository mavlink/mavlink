...
static inline uint16_t mavlink_msg_statustext_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, uint8_t severity, const char* text)
{
	...
	i += put_array_by_index((const int8_t*)text, sizeof(char)*50, i, msg->payload); // Status text message, without null termination character
 ...
 */
static inline uint16_t mavlink_msg_statustext_get_text(const mavlink_message_t* msg, char* r_data)
{

	memcpy(r_data, msg->payload+sizeof(uint8_t), sizeof(char)*50);
