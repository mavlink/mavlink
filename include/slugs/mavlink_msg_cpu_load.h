						       ...
						       uint8_t sensLoad, uint8_t ctrlLoad, uint16_t batVolt)
{
	...
	put_uint16_t_by_index(batVolt, 0,  msg->payload); // Battery Voltage in millivolts
	put_uint8_t_by_index(sensLoad, 2,  msg->payload); // Sensor DSC Load
	put_uint8_t_by_index(ctrlLoad, 3,  msg->payload); // Control DSC Load

	return mavlink_finalize_message(msg, system_id, component_id, 4, 242);
}
						           ...
						           uint8_t sensLoad,uint8_t ctrlLoad,uint16_t batVolt)
{
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 4, 242);
}
						           ...
						           uint8_t sensLoad,uint8_t ctrlLoad,uint16_t batVolt)
{
	...
	mavlink_finalize_message_chan_send(msg, chan, 4, 242);
}
