						       ...
						       uint8_t target_system_id, uint16_t watchdog_id, uint16_t process_id, uint8_t command_id)
{
	...
	put_uint8_t_by_index(target_system_id, 4,  msg->payload); // Target system ID
	put_uint8_t_by_index(command_id, 5,  msg->payload); // Command ID

	return mavlink_finalize_message(msg, system_id, component_id, 6, 68);
}
						           ...
						           uint8_t target_system_id,uint16_t watchdog_id,uint16_t process_id,uint8_t command_id)
{
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 6, 68);
}
						           ...
						           uint8_t target_system_id,uint16_t watchdog_id,uint16_t process_id,uint8_t command_id)
{
	...
	mavlink_finalize_message_chan_send(msg, chan, 6, 68);
}
