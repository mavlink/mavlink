						       ...
						       uint8_t target_system, uint8_t target_component, float x, float y, float z, float yaw)
{
	...
	put_float_by_index(x, 0,  msg->payload); // x position offset
	put_float_by_index(y, 4,  msg->payload); // y position offset
	put_float_by_index(z, 8,  msg->payload); // z position offset
	put_float_by_index(yaw, 12,  msg->payload); // yaw orientation offset in radians, 0 = NORTH
	...
	return mavlink_finalize_message(msg, system_id, component_id, 18, 105);
}
						           ...
						           uint8_t target_system,uint8_t target_component,float x,float y,float z,float yaw)
{
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 18, 105);
}
						           ...
						           uint8_t target_system,uint8_t target_component,float x,float y,float z,float yaw)
{
	...
	mavlink_finalize_message_chan_send(msg, chan, 18, 105);
}
