 ...
 */
static inline uint16_t mavlink_msg_sys_status_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, uint8_t mode, uint8_t status, uint16_t vbat, uint8_t motor_block, uint16_t packet_drop)
{
...
{
	return mavlink_msg_sys_status_pack(system_id, component_id, msg, sys_status->mode, sys_status->status, sys_status->vbat, sys_status->motor_block, sys_status->packet_drop);
}

#ifdef MAVLINK_USE_CONVENIENCE_FUNCTIONS

static inline void mavlink_msg_sys_status_send(mavlink_channel_t chan, uint8_t mode, uint8_t status, uint16_t vbat, uint8_t motor_block, uint16_t packet_drop)
{
	mavlink_message_t msg;
	mavlink_msg_sys_status_pack(mavlink_system.sysid, mavlink_system.compid, &msg, mode, status, vbat, motor_block, packet_drop);
